...
class ConcolicExecutor:
    ...
    def run(self, timeout=None):
        ...
        if stdin:
            symsan.config("stdin", plan_file=plan_fp, args=cmd, debug=logging_level, bounds=shoud_trace_bounds)
            symsan.reset_input([self.input_content])
            symsan.run(stdin=self.cur_input)
        else:
            symsan.config(self.cur_input, plan_file=plan_fp, args=cmd, debug=logging_level, bounds=shoud_trace_bounds)
            symsan.reset_input([self.input_content])
            symsan.run()
